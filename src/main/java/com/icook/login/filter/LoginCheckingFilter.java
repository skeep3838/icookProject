package com.icook.login.filter;
...
@WebFilter(
		urlPatterns = { "/*" }, 
		initParams = { 
				@WebInitParam(name = "mustLogin1", value = "/courseOrder/*"),
				@WebInitParam(name = "mustLogin2", value = "/MyOrders/*"),
				@WebInitParam(name = "mustLogin3", value = "/recipes/add"),
				@WebInitParam(name = "mustLogin3", value = "/recipes/list"),
				@WebInitParam(name = "mustLogin5", value = "/shoppingCart/*")
//				@WebInitParam(name = "mustLogin4", value = "/courseOrder/*")
		})
...
//			System.out.println("進入filter，URI: "+requestURI);
			if (mustLogin()) {           
				if (checkLogin(req)) {   
					//  需要登入，但已經登入
					...
					resp.sendRedirect(contextPath + "/login/loginView");
					return;
				}
